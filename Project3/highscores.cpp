...
void initHighscoreTable(void)
{
	...
	for (i = 0; i < NUM_HIGHSCORES; i++)
	{
		...
		strncpy_s(highscores.highscore[i].name, 16, "REGISTER", MAX_SCORE_NAME_LENGTH);
	}
...
static void doNameInput(void)
{
	...
	if (app.keyboard[SDL_SCANCODE_RETURN])
	{
		...
		{
			strncpy_s(newHighscore->name, 16, "REGISTER", MAX_SCORE_NAME_LENGTH);
		}
...
{
	FILE* file;
	errno_t err = fopen_s(&file, "highscores.txt", "w");
	if (err != 0 || file == NULL)
	{
		SDL_Log("Failed to open highscores.txt for writing");
		return;
	}
...
{
	FILE* file;
	errno_t err = fopen_s(&file, "highscores.txt", "r");
	if (err != 0 || file == NULL)
	{
		SDL_Log("Failed to open highscores.txt for reading");
		return;
	}

	for (int i = 0; i < NUM_HIGHSCORES; i++)
	{
		if (fscanf_s(file, "%15s %d", highscores.highscore[i].name, (unsigned int)sizeof(highscores.highscore[i].name), &highscores.highscore[i].score) != 2)
		{
			break;
		}
	}
